...
pub struct ContextBuildResult {
    ...
    /// Used for provenance tracking and `SelectedFor` edge creation.
...
impl ContextPolicy {
    ...
    /// Conversational: active branch leaf. Task: the work item's chain leaf.
    pub fn initial_parent(&self, graph: &ConversationGraph) -> anyhow::Result<Uuid> {
        match self {
            Self::Conversational => graph.active_leaf(),
            Self::TaskExecution { work_item_id } => Ok(graph.find_chain_leaf(*work_item_id)),
        }
    }

    /// Record an assistant message in the graph using the appropriate method.
    /// Conversational: `add_message` (updates branch). Task: `add_reply` (no branch).
